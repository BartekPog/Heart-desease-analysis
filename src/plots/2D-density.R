...
generate2DDensity <- function(data){
  ...
  #Age vs blood pressure
    ggplot(dataClean,aes(age,trestbps)) +
  ...
  #Age vs heart rate
    ggplot(dataClean,aes(age,thalach)) +
        stat_density_2d(aes(fill = ..level..), geom = "polygon")+
  ...
  #Heart rate vs blood pressure
    ggplot(dataClean,aes(thalach,trestbps)) +
        stat_density_2d(aes(fill = ..level..), geom = "polygon")+
  ...
  #Heart rate vs cholesterol
    ggplot(dataClean,aes(thalach,chol)) +
        stat_density_2d(aes(fill = ..level..), geom = "polygon")+
        ggtitle("Wykres gęstości tętna względem ciśnienia krwi") + 
        labs(y="Cholesterol")+
        labs(x="Tętno")+
  ...
  #Cholesterol vs blood pressure
    ggplot(dataClean,aes(chol,trestbps)) +
        stat_density_2d(aes(fill = ..level..), geom = "polygon")+
        ggtitle("Wykres gęstości tętna względem ciśnienia krwi") + 
        labs(y="Ciśnienie")+
        labs(x="Tętno")+
  ...
  #Oldpeak vs blood pressure
    ggplot(dataClean,aes(oldpeak,trestbps)) +
        stat_density_2d(aes(fill = ..level..), geom = "polygon")+
  ...
  #Oldpeak vs heart rate
    ggplot(dataClean,aes(oldpeak,thalach)) +
        stat_density_2d(aes(fill = ..level..), geom = "polygon")+
  ...
  #Oldpeak vs cholesterol
    ggplot(dataClean,aes(oldpeak,chol)) +
        stat_density_2d(aes(fill = ..level..), geom = "polygon")+
